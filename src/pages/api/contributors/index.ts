import type { NextApiRequest, NextApiResponse } from 'next';
import { roqClient } from 'server/roq';
import { prisma } from 'server/db';
import { authorizationValidationMiddleware, errorHandlerMiddleware } from 'server/middlewares';
import { contributorValidationSchema } from 'validationSchema/contributors';
import { convertQueryToPrismaUtil } from 'server/utils';
import { getServerSession } from '@roq/nextjs';

async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { roqUserId, user } = await getServerSession(req);
  switch (req.method) {
    case 'GET':
      return getContributors();
    case 'POST':
      return createContributor();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getContributors() {
    const data = await prisma.contributor
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'contributor'));
    return res.status(200).json(data);
  }

  async function createContributor() {
    await contributorValidationSchema.validate(req.body);
    const body = { ...req.body };
    if (body?.literature?.length > 0) {
      const create_literature = body.literature;
      body.literature = {
        create: create_literature,
      };
    } else {
      delete body.literature;
    }
    const data = await prisma.contributor.create({
      data: body,
    });
    return res.status(200).json(data);
  }
}

export default function apiHandler(req: NextApiRequest, res: NextApiResponse) {
  return errorHandlerMiddleware(authorizationValidationMiddleware(handler))(req, res);
}
